...
def _process_pending_concurrent(
    ...
    *,
    ...
    """Execute local transactions concurrently while mailbox ownership stays serial."""
    ...
    try:
        for filename in still_pending:
            ...
            try:
                ...
                if job.kind == "transaction":
                    while True:
                        # Registry/config resolution is frozen immediately before handoff.
                        registry = load_registry()
                        worker_task = _prepare_transaction_worker_task(cfg, registry, request)
                        if (
                            scheduler.active_count() < scheduler.max_workers
                            and not scheduler.repo_in_flight(worker_task.scheduling_key)
                        ):
                            break
                        if not active:
                        ...
                        same_repo = next(
                            (
                                item
                                for item in active
                                if item.scheduling_key == worker_task.scheduling_key
                            ),
                            None,
                        )
                        drain_item(same_repo or active[0])

                    handle = scheduler.submit(worker_task)
                    active.append(_InFlightTransaction(
                        request=request,
                        handle=handle,
                        request_started=request_started,
                        scheduling_key=worker_task.scheduling_key,
                    ))
                    continue

                if job.kind == "materialize":
                    # Materialisation remains watcher-owned and uses a quiescent barrier in C1.
                    drain_all()
        ...
        # A transport/result-publication failure must never strand scheduler
        # ownership. Reap every submitted worker; each completed worker result is
        # durably persisted before its serial publication attempt can fail.
        while active:
            item = active[0]
            try:
                drain_item(item)
            except BaseException:
                # Continue reaping the remaining workers, then preserve the first
                # exception from the main processing path for the watch loop.
                continue
        raise
