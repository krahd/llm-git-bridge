...
`llm-git-bridge` `0.3.1` supports multiple remote writers, including several ChatGPT sessions, but deliberately remains a **single-consumer, one-worker** local system. The frozen `0.3.0` release is the semantic oracle; `0.3.1` introduces the explicit watcher/scheduler/worker boundary while preserving one-worker behaviour. Real cross-repository parallel execution is not enabled yet.
