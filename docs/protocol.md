...
A transport failure while downloading the transaction object is **not** an application-level rejection: the daemon has not yet obtained request bytes it can validate. Such failures leave the inbox object pending and are retried on a later poll without publishing a result. Once bytes have been downloaded, deterministic content/protocol errors (for example an oversized or invalid JSON request) produce the normal signed terminal error result.

The remote result is the durable acknowledgement. Results carry a local-key HMAC authenticator; replay recovery trusts a remote acknowledgement only when that authenticator verifies. The HMAC key is durable user-private configuration so deletion of disposable runtime state does not silently invalidate legitimate historical acknowledgements. A mailbox writer can read result tags but cannot forge a valid acknowledgement for a new result without the local key. This authenticator is a bridge-side replay/reconciliation control: ordinary remote clients do not receive the private key and therefore must not claim independent cryptographic verification of `bridge_auth` unless a trusted verifier actually performs it. Once that acknowledgement exists, the original transaction object is no longer durable state: the daemon best-effort deletes it from the transaction inbox and also reaps previously acknowledged requests during idle polls. This keeps the hot polling directory small without weakening replay protection; local request/result copies and the remote result remain available for diagnosis. The daemon also keeps a local published-result marker. At watcher startup it reconciles the remote result directory once and rebuilds any missing local markers before processing transactions. Normal polling then lists only the transaction directory, preserving replay safety across restarts without adding a result-directory listing to every first-seen transaction. Results include transport timings known before acknowledgement and identify whether transaction listing/download used persistent `rclone rcd` or the subprocess fallback. Result-upload duration and its transport mode are recorded in the daemon's local JSONL metrics.
