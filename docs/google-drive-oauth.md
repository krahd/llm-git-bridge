...
Afterward, Shell Bridge `doctor` should report `private_oauth_client_configured: true`. The diagnostic inspects `rclone config redacted` and returns only a boolean; it never returns the client ID, client secret, or token.
