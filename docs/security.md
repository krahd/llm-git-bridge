...
A concurrent worker exception is converted into that transaction's error result and does not terminate sibling workers or the watcher. If result publication fails after local work completed, every submitted worker is reaped and its signed result is persisted locally before the watcher unwinds. A subsequent watcher instance republishes those durable results instead of re-executing the mutation. Correctness therefore does not depend on reconstructing an in-memory scheduler queue after restart.
