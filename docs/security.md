...
`add-root` is a local authorization decision. RC5 treats every configured root as an ongoing discovery boundary: the watcher may expose path-free metadata for valid Git working trees found beneath it without another per-repository registration step. It never broadens the roots itself and does not follow hidden/build/cache subtrees that the scanner excludes.
...
Repository IDs are local policy identities: configured commands and push permission are keyed by ID. The local registry therefore binds an ID to a logical Git-history identity and retains retired IDs. If a different repository replaces a registered working tree, a full authoritative refresh retires the prior ID and assigns the replacement a fresh ID so it cannot inherit stale command or push privileges. Identity is rechecked immediately before repository-dependent work. Local marker/history identities and the retired-ID ledger are never included in the public registry or diagnostics.
