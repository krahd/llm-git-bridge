...
The watcher publishes `v2/meta/repos.json` only when repository membership/name-location changes, then atomically replaces the local registry. Diagnostics expose only bounded counters/timings. Repository workers never mutate the registry and continue to receive frozen repository/config inputs at handoff.

A repository identity mismatch is treated differently from an ordinary periodic scan: it is evidence that local repository history changed beneath an existing policy ID. The watcher performs one full authoritative refresh, retires the old ID when appropriate, republishes the path-free registry, and replans against the new ID. This protects command/push policy from path-replacement inheritance without allowing arbitrary remote requests to trigger repeated full scans.
