...
Version `1.0.0rc5` retains the RC4/Phase B ownership boundary while allowing a bounded number of repository-local workers and adds watcher-owned automatic repository discovery beneath approved roots. Drive/rclone transport, registry mutation and durable result publication remain watcher-owned; the scheduler owns worker lifecycle, canonical-repository exclusion, bounded backlog admission and fairness. The complete ownership and recovery contract is documented in `scheduler-architecture.md`.
...
## RC5 automatic repository discovery

Repository roots remain operator-configured local state. The single watcher owns periodic membership discovery as another control-plane responsibility. At the start of an idle mailbox cycle, and on schedule while long-running workers are active, it performs a lightweight filesystem scan below approved roots. Known repositories are matched from local registry state without spawning Git for every entry; newly seen `.git` candidates are validated with Git before they can enter the registry.

The watcher publishes `v2/meta/repos.json` when repository membership/name-location changes; local policy-changing commands also republish it immediately so effective `read`/`edit`/`push` capabilities do not remain stale. The public index never contains root or repository paths. Diagnostics expose only bounded counters/timings. Repository workers never mutate the registry and continue to receive frozen repository/config inputs at handoff.

A repository identity mismatch is treated differently from an ordinary periodic scan: it is evidence that local repository history changed beneath an existing policy ID. The watcher performs one full authoritative refresh, retires the old ID when appropriate, republishes the path-free registry, and replans against the new ID. This prevents configured commands and per-repository overrides from transferring to unrelated replacement history. Root-level policy still applies intentionally to every repository beneath its approved boundary.
